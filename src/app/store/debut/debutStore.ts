...
export class DebutStore {
  ...
  async deleteMove(){
    ...
    try {
      ...
      this.debutSignal.update(prev=>({...prev, firstMoves: prev.firstMoves.filter(el=>el.id!==this.selectedMove().id)}))
      this.closeModal()
    } catch (err: unknown) {
      handleHttpError(this.global, err, 'Не удалось добавить ход')
    } finally {
  ...
  async updateMove(desc:string){
    ...
    try {
      ...
      this.debutSignal.update(prev=> ({...prev, firstMoves: prev.firstMoves.map(el=>{
