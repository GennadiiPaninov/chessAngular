...
export function validateMoveHelper(
  ...
  return (group: AbstractControl): ValidationErrors | null => {
    ...
    const mFrom = isWhite
      ? group.get(mFromKey)?.value?.toLowerCase()
      : group.get(eFromKey)?.value?.toLowerCase();
    const mTo = isWhite
      ? group.get(mToKey)?.value?.toLowerCase()
      : group.get(eToKey)?.value?.toLowerCase();
    const eFrom = isWhite
      ? group.get(eFromKey)?.value?.toLowerCase()
      : group.get(mFromKey)?.value?.toLowerCase();
    const eTo = isWhite
    ...
    if (mFrom.length !== 2 || mTo.length !== 2 || eFrom.length !== 2 || eTo.length !== 2)
      return null;
    ...
    try {
      ...
      if (!move2) {
        ...
        updateNewMove({
          fen: chess.fen(),
          fens: [fenAfterFirst, chess.fen()],
          pieces: movedPiecesArr,
        });
      }
    } catch (e) {
      eToControl?.setErrors({ ...eToControl.errors, ['invalidMove']: true });
